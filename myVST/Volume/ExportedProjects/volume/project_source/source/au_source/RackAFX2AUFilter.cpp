	...
	if (inScope == kAudioUnitScope_Global)
	{
		switch(inID)
		{
			...
			case kAudioUnitProperty_CocoaUI:
			{
				...
				CFBundleRef bundle = CFBundleGetBundleWithIdentifier( CFSTR("developer.audiounit.volume") );

				if(bundle == NULL) return fnfErr;

				CFURLRef bundleURL = CFBundleCopyResourceURL(bundle,
                                                             CFSTR("volume_AU"),	// this is the name of the cocoa bundle as specified in the CocoaViewFactory.plist
                                                             CFSTR("bundle"),			// this is the extension of the cocoa bundle
                                                             NULL);

                if(bundleURL == NULL) return fnfErr;

				CFStringRef className = CFSTR("volume610244953_ViewFactory");	// name of the main class that implements the AUCocoaUIBase protocol
